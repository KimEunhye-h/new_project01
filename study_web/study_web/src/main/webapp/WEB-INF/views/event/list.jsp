	...
	<div class="subContainer" id="contents">
		<div class="inner1220">
			...
			<section class="sSubConSection">
				...
				<div class="subTitle">
					<h2>행사 및 전시</h2>
					<form method="post" action='list.ev' id="list">
							...
							<fieldset class="searchmenu">
								...
								<ul>
									<c:if test="${login_info.admin eq 'y' }">
										<li><a class="btn-fill" href='new.ev'>글쓰기</a></li>
									</c:if>
								</ul>
							</fieldset>
						</div>
					...
					<table>
						<p class="event_list">각종 행사 및 전시 리스트</p>
						<c:forEach items="${page.list }" var="vo">
							<ul class="img_ul">
								<li>
								<div class="img_title" style=""><c:if
										test="${!empty vo.gdsImg}">
										<a href="download.ev?id=${vo.id}"> 
								...
								<div class="left img_con"><a
									...
									title="${vo.content}"
									href='detail.ev?id=${vo.id}&curPage=${page.curPage}'>${vo.title }</a><br>
									[ ${vo.writedate }]<br>
								<br>
