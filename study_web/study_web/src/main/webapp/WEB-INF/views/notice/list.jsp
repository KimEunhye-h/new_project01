				...
				<div class="subTitle">
					...
					<table>
						...
						<c:forEach items="${page.list }" var="vo">
							<tr>
								<td>${vo.no}</td>
								<td class="left"><c:forEach var="i" begin="1" end="${vo.indent}"> ${i eq vo.indent ? "<img src='img/re.gif'/>" : '&nbsp;&nbsp;'}
								</c:forEach> <a title="${vo.content}" href='detail.no?id=${vo.id}&curPage=${page.curPage}'>${vo.title }</a></td>
								<td>${vo.name }</td>
								<td>${vo.writedate }</td>
								<td><c:if test="${!empty vo.filename}">
										<a href="download.no?id=${vo.id}"> <img
