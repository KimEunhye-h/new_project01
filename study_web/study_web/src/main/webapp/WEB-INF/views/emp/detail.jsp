...
<body>
	<div class="subContainer" id="contents">
		<div class="inner1220">
			...
			<aside class="sAsideMenu">
				...
				<ul class="sAsideMul">
					...
					<li class="active"><a
						href="javascript:location.href='list.emp';"
				...
				<div class="subTitle">
					...
					<table>
						<tr>
							...
							<td class="left" colspan="4">${vo.title }</td>
							<th>작성자</th>
							<td>${vo.name}</td>
		...
		<span id="preview"></span>
									<a href="download.emp?id=${vo.id }"><i
					...
					<div class="btnSet">
						...
						<c:if
							...
							<a class="btn-fill"
								onclick="$('form').attr('action','modify.emp');$('form').submit()">수정</a>
							<a class="btn-fill"
								onclick="if(confirm('정말 삭제?')){ $('form').attr('action','delete.emp'); $('form').submit() }">삭제</a>
						</c:if>
					</div>
					...
					<form action="list.emp" method="post">
