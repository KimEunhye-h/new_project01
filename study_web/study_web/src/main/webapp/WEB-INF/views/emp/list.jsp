	...
	<div class="subContainer" id="contents">
		<div class="inner1220">
			...
			<section class="sSubConSection">
				...
				<div class="subTitle">
					<h2>직장인</h2>
					<form id="list" action="list.emp" method="post">
							...
							<fieldset class="searchmenu">
								...
								<ul class="searchbtn">
									...
									<c:if test="${!empty login_info}">
										<!-- 로그인되어 있으면 글쓰기 가능 -->
										<li><a class="btn-fill" href="new.emp">글쓰기</a></li>
									</c:if>
								</ul>
							</fieldset>
						</div>
							...
							<ul class="grid">
								<c:forEach items="${page.list}" var="vo">
									<li>
										...
										<div>${vo.name}</div>
										<div>${vo.writedate}<span>${empty vo.filename ? '':'<img src="img/attach.png" class="file-img"/>'}</span>
										</div>
									</li>
								</c:forEach>
							</ul>
